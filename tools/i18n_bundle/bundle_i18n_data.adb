...
--  crate's share tree, so a deployed bin/devcert finds it at Exe/../share/i18n.
...
--  builds with plain gprbuild and stays out of the app's bin; the devcert
--  post-build actions build then run it (see alire.toml). It runs from the
--  devcert crate root, so the relative paths below resolve there. Idempotent: it
